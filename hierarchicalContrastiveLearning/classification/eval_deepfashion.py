...
def set_model(args):
    ...
    ##state_dict = ckpt['state_dict'] ##comment 13.07.23
    state_dict = model.state_dict() ##add 13.07.23

    if torch.cuda.is_available():
        if torch.cuda.device_count() > 1:
            new_state_dict = {}
            for k, v in state_dict.items():
                #k = k.replace("module.encoder", "encoder.module") ##comment 13.07.23
                k = k.replace("encoder", "encoder.module") ##add 13.07.23
                k = k.replace("encoder.module.module", "encoder.module") ##add 13.07.23 knov
                if k.startswith("module.head"):
                    k = k.replace("module.head", "head")
                new_state_dict[k] = v
            state_dict = new_state_dict
        ##add 13.07.23 knov
        print("keys change begin")
        new_state_dict = {}
        for k, v in state_dict.items():
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, args):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        labels0=torch.stack(labels)
        print("1-train labels0 shape: ", labels0.shape) ##del knov 
        labels = torch.stack(labels, dim=1)[:, 0]
        print("1-train labels0_2 shape: ", labels.shape) ##del knov 
        ...
        print("1-train features shape: ", features.shape) ##del knov    
        output = classifier(features.detach())
        loss = criterion(output, labels)

        # update metric
        losses.update(loss.item(), bsz)
        print("1-train labels shape: ", labels.shape) ##del knov
        print("1-train output shape: ", output.shape) ##del knov
...
def validate(val_loader, model, classifier, criterion, args):
    ...
    with torch.no_grad():
        ...
        for idx, (images, labels) in enumerate(val_loader):
            ...
            print("2-val labels shape: ", labels.shape) ##del knov
            print("2-val output shape: ", output.shape) ##del knov
            # update metric
            losses.update(loss.item(), bsz)
            acc1, acc5 = accuracy(output, labels, topk=(1, 5))
            top1.update(acc1[0], bsz)
...
def test(test_loader, model, classifier, criterion, args):
    ...
    with torch.no_grad():
        ...
        for idx, (images, labels) in enumerate(test_loader):
            ...
            print("3-test labels shape: ", labels.shape) ##del knov
            print("3-test output shape: ", output.shape) ##del knov
...
def load_deep_fashion_hierarchical(root_dir, train_list_file, val_list_file, test_list_file, class_map_file, repeating_product_file, input_size, batch_size,crop_size):
    ...
    print(root_dir)##del knov
    print(test_list_file) ##del knov
    print(os.path.join(root_dir, train_list_file)) ##del knov
    ...
    dataloaders_dict = {
     ...
     ###knov begin
    dataloaders_dict1 =  torch.utils.data.DataLoader(image_datasets['train'],
                                       num_workers=16, batch_size=256,
                                       shuffle=False)
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        pred = pred.t()
        print("acc func") ##del knov
        print("pred shape: ", pred.shape) ##del knov
        print("target shape: ", target.shape) ##del knov
        print(target.view(1, -1).shape) ##del knov
        print(pred.eq(target.view(1, -1).expand_as(pred))) ##del knov
