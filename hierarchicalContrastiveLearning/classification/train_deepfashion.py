...
def set_model(ngpus_per_node, args):
    ...
    #state_dict = ckpt['model'] ##commented 28.06.23
    state_dict = model.state_dict() ##add 28.06.23
...
def train(dataloaders, model, criterion, optimizer, epoch, args, logger):
    ...
    for phase in ['train']:
        ...
        for idx, (images, labels) in enumerate(dataloaders[phase]):
            ...
            images0 = torch.stack(images) ##del knov
            print("train images0 shape: ", images0.shape) ##del knov
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                print("train features shape: ", features.shape) ##del knov 
...
def load_deep_fashion_hierarchical(root_dir, train_list_file, val_list_file, class_map_file, repeating_product_file, opt):
    ...
    print("13")
    print(os.path.join(root_dir, train_list_file))
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        res = []
        for k in topk:
            #correct_k = correct[:k].view(-1).float().sum(0, keepdim=True) ## comment knov
            correct_k = correct[:k].reshape(-1).float().sum(0, keepdim=True) # add knov
